...
import { signIn, signInWithGoogle } from "../Handlelogin";
...
const LoginScreen = () => {
  ...
  return (
    <View className='m-4'>
      ...
      <View className='bg-slate-300 h-px w-80 mt-4 ml-4'></View>
      ...
      <TouchableOpacity className='bg-blue-900 rounded-3xl w-80 h-10 ml-4  flex-row'>
        ...
        <Text className='text-white ml- text-lg mt-1'>
          {" "}
      ...
      <TouchableOpacity
        className='border-2 rounded-3xl w-80 h-10 ml-4 mt-4 flex-row'
        onPress={() => {
          signInWithGoogle();
          setIsLoading(!isLoading);
        }}>
        ...
        <Text className=' ml- text-lg mt-1'>
          {" "}
